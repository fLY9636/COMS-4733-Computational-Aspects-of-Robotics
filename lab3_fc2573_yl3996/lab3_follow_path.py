...
class Lab():
    ...
    def shortest_path(self):
       ...
       # print(self.edges)
        ...
        while(start != end):
            self.shortest_path.append(self.obstacles_oneD[start])
            start = self.path[start][end]
        self.shortest_path.append(self.obstacles_oneD[end])
        ...
        for item in self.shortest_path:
    ...
    # control the robot to move forward {meter}
        ...
        # move backward if the target distance is small then 0
        ...
        # calculate the angle
        diff_x = goal_point.x - position.x
        diff_y = goal_point.y - position.y

        goal_angle = np.arctan(diff_y/diff_x) - rotation 
        ...
        print('stop rotation: begin farward')
        ...
        distance = 0
        # target distance
        goal_distance = sqrt(pow((goal_point.x - x_start), 2) +                                                                               
                            pow((goal_point.y - y_start), 2))

        print('target distance:', goal_distance)
    ...
    def follow_path(self):
        ...
        for i in range(1, len(self.shortest_path)):
            self.move_next(self.shortest_path[i])
